...
class Lyngsat():
	...
	# 通过Degree，Frequency，SID查找Channel Name
	def lookup_all_chn(self, sdg, sfq, sid):
		urls_sats =  self.lookup_sat_urls(sdg)
		result = False
		chn_name = "No dgr found"
		for urls_sat in urls_sats:
			(result, chn_name) = self.lookup_chn_in_sat(urls_sat, sfq, sid)
			if result:break
		logger.info('>> chn name:' + chn_name)
		return (result, chn_name)
				...
				for tr in trs:
					...
					if len(tds) == 10 and len(tds[0].text) > 2:
						...
						if chn_freq_content:
							...
							try:
								chn_freq_content_float = float(chn_freq_content)
							except Exception,e:
								chn_freq_content_float = 0
							if chn_freq_content_float and chn_freq_content_float== float(freq):
								chn_freq = str(chn_freq_content)
								chn_sid_content = tds[6].text
								chn_sid = chn_sid_content.replace('&nbsp;','').replace(' ','')
								try:
									chn_sid_int = int(chn_sid)
								except Exception,e:
									chn_sid_int = 0

								if chn_sid_int and chn_sid_int == int(sid):
									chn_name = tds[2].text
									break
							else:continue
					elif len(tds) == 9:
						...
						for td in tds:
							...
							if td.attrs and ('bgcolor','#99ff99') in td.attrs:
								chn_info.append(td.text)
						if len(chn_info):
							chn_sid = chn_info[4].replace('&nbsp;','').replace(' ','')
							try:
								chn_sid_int = int(chn_sid)
							except Exception,e:
								chn_sid_int = 0
...
if __name__ == '__main__':
	...
	urls_sats =  lyngsat.lookup_sat_urls('19.2E')
        ...
        chn_name = ''
	for urls_sat in urls_sats:
		(result, chn_name) = lyngsat.lookup_chn_in_sat(urls_sat, 11126, 30860)
		if result:break
	logger.info('>> chn name:' + chn_name)
